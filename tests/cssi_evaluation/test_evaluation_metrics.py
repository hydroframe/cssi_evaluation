...
def test_rmse():
    ...
    calc = metric_utils.RMSE(obs, sim)
...
def test_pearson_r():
    ...
    calc = metric_utils.pearson_R(obs, sim)
...
def test_spearman_rank():
    ...
    calc = metric_utils.spearman_rank(obs, sim)
...
def test_r_squared():
    ...
    calc = metric_utils.R_squared(obs, sim)
...
def test_bias_from_r():
    ...
    calc = metric_utils.bias_from_R(obs, sim)
    assert math.isclose(
...
def test_bias():
    ...
    calc = metric_utils.bias(obs, sim)
    assert math.isclose(
...
def test_percent_bias():
    ...
    calc = metric_utils.percent_bias(obs, sim)
    assert math.isclose(
...
def test_absolute_relative_bias():
    ...
    calc = metric_utils.absolute_relative_bias(obs, sim)
    assert math.isclose(
        calc, 0.15384615, rel_tol=0.000001
    )  # value aligned with using R HydroGOF package
...
def test_total_difference():
    ...
    calc = metric_utils.total_difference(obs, sim)
